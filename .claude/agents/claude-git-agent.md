---
name: claude-git-agent
description: claude-target documentation for the git-agent push step. The CI workflow in `.github/workflows/build-and-security.yml` is the only writer and computes the next chain step itself — the model is not used in this step. This file documents the rules the workflow enforces for the `feature/claude-git-agent_<N>_<TIMESTAMP>` chain.
---

# claude Git Agent — `feature/claude-git-agent` Chain Step (workflow-computed, no model)
...
This file is **documentation only**. The workflow does not POST to claude for this step.
...
```
feature/claude-git-agent_<N>_<TIMESTAMP>
```
...
- The development branch `feature/claude-git-agent` is the **bootstrap parent** of push #1.

## Counter rule

```
MAX_N = max(N over all remote+local refs matching `feature/claude-git-agent_[0-9]+_*`)
NEW_N = MAX_N + 1
```
...
```bash
git ls-remote --heads origin 'feature/claude-git-agent_*'
git branch --list 'feature/claude-git-agent_*'
```
...
```
feature/claude-git-agent                                ← bootstrap (development branch)
   └─ feature/claude-git-agent_1_<ts1>                  ← push #1
         └─ feature/claude-git-agent_2_<ts2>            ← push #2
               └─ feature/claude-git-agent_3_<ts3>      ← push #3
                     └─ ...
```

- The **parent** of push #N is the existing `feature/claude-git-agent_<MAX_N>_<its-timestamp>` with the largest N (combined remote + local).
- If no `feature/claude-git-agent_<N>_*` ref exists yet, **N = 1** and **parent = `feature/claude-git-agent`** (the development branch itself).
...
1. **Branch format:** `^feature/claude-git-agent_[0-9]+_[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2}$`
...
5. **Reports stay on the development branch:** after the push, `SECURITY_ASSESSMENT_REPORT.md` and `SECURE_REMEDIATION_REPORT.md` are committed back to `feature/claude-git-agent` (the trigger branch), not to the chain step. The chain-step branch is rejected as a target.

## Commit message shape

```
claude git-agent push #<N> — automated remediation
...
Manual merge target: feature/claude-git-agent (then main, human review required)
...
- `push` to `feature/claude-git-agent`
- `pull_request` targeting `feature/claude-git-agent`
- `workflow_dispatch` (manual run from the Actions tab)

It does **not** fire on pushes to `feature/safe-backup_*`, `feature/claude-git-agent_<N>_<TS>` chain steps, or any other branch.
...
- Push branch family: was `feature/safe-backup_<N>_<TS>`, now `feature/claude-git-agent_<N>_<TS>`.
- Bootstrap parent: was `feature/safe-backup`, now `feature/claude-git-agent` (the development branch).
- Counter scope: was `feature/safe-backup_*`, now `feature/claude-git-agent_*`.
- Subject prefix: was `Safety backup push #`, now `claude git-agent push #`.
